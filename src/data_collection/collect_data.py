...
if not IS_WEB:
    try:
        ...
        print(
            "[WARNING] OpenCV (cv2) not available. Image processing features will be limited."
        )
        CV2_AVAILABLE = False
else:
    print("[INFO] Web environment detected. OpenCV features not available.")
    CV2_AVAILABLE = False
...
class DataCollector:
    ...
    def find_bbox_by_color(self, image, lower_color, upper_color):
        if not CV2_AVAILABLE:
            print(
                "[WARNING] OpenCV not available. Color-based bbox detection not supported."
            )
            return []
    ...
    def visualize_bboxes(self, frame, auto_bboxes, manual_bboxes):
        if not CV2_AVAILABLE:
            print("[INFO] OpenCV not available. Visualization skipped.")
            return
    ...
    def collect_frame(self):
        ...
        if CV2_AVAILABLE:
            ...
            # PIL을 사용한 이미지 저장 (대체 방법)
            try:
                from PIL import Image

                # BGR을 RGB로 변환
                if len(frame.shape) == 3 and frame.shape[2] == 3:
                    rgb_frame = frame[:, :, ::-1]  # BGR을 RGB로 변환
                else:
                    rgb_frame = frame
                pil_image = Image.fromarray(rgb_frame.astype(np.uint8))
                pil_image.save(image_path)
            except ImportError:
                print("[ERROR] Neither OpenCV nor PIL is available for image saving.")
                return frame
        # 라벨 생성 및 저장
        labels = []
        auto_bboxes = []
        ...
        for cls, pos, bbox in labels:
            ...
            yolo_labels.append(
                f"{cls} {norm[0]:.6f} {norm[1]:.6f} {norm[2]:.6f} {norm[3]:.6f}"
            )
        ...
        manual_label_path = os.path.join(
            r"c:/Users/easy/Desktop/1/images", label_filename
        )
        ...
        manual_bboxes = self.load_manual_bboxes(
            manual_label_path, img_width, img_height
        )
    ...
    try:
        while True:
            ...
            if CV2_AVAILABLE:
                ...
                # OpenCV가 없으면 간단한 로깅으로 대체
                print(f"Frame {collector.frame_count} collected (shape: {frame.shape})")
                time.sleep(0.1)  # 프레임 레이트 제한
