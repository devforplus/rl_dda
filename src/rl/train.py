...
def parse_args():
    """커맨드 라인 인자 파싱"""
    parser = argparse.ArgumentParser(description='TorchRL을 사용한 랜덤 에이전트 학습')
...
def save_metrics(metrics: Dict[str, Any], log_dir: str) -> None:
    """학습 메트릭 저장"""
    ...
    plt.xlabel('Episode')
    plt.ylabel('Steps')
    plt.savefig(os.path.join(log_dir, 'lengths.png'))
    plt.close()

def train_agent(args):
    """랜덤 에이전트 학습"""
    print("=== 개선된 TorchRL 기반 랜덤 에이전트 학습 시작 ===")
    ...
    try:
        ...
        # 에이전트 평가 실행
        results = agent.evaluate(num_episodes=args.episodes)
        
        # 메트릭 업데이트
        metrics['episode_rewards'] = results['rewards']
        metrics['episode_lengths'] = results['lengths']
        metrics['mean_reward'] = results['mean_reward']
        metrics['mean_length'] = results['mean_episode_length']
        metrics['total_time'] = time.time() - metrics['start_time']
        
        # 결과 출력
        print("\n===== 학습 결과 =====")
        print(f"평균 보상: {metrics['mean_reward']:.2f}")
        print(f"평균 에피소드 길이: {metrics['mean_length']:.2f}")
        print(f"총 소요 시간: {metrics['total_time']:.2f}초")
        
        # 메트릭 저장
        save_metrics(metrics, args.log_dir)
