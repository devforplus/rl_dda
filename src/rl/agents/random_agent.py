...
class RandomAgent(BaseAgent):
    ...
    def select_action(self, state):
        ...
        """
        if hasattr(self.action_space, "sample") and callable(
            getattr(self.action_space, "sample")
        ):
            return self.action_space.sample()
        elif isinstance(self.action_space, list):
            if not self.action_space:
                raise ValueError(
                    "Action space is an empty list. Cannot select an action."
                )
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        print(
            f"Trial {i + 1}: Selected action ID: {selected_action_id}, Meaning: {action_names[selected_action_id]}"
        )
