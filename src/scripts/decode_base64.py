...
def decode_base64_to_image_file(base64_string, output_filename="decoded_image.png"):
    ...
    except Exception as e:  # binascii.Error를 포함한 모든 디코딩 오류 처리
        print(f"Base64 디코딩 오류: {e}")
        print("입력된 문자열이 올바른 Base64 형식이 아니거나 손상되었을 수 있습니다.")
    except IOError as e:
        print(f"이미지 데이터 오류: {e}")
        print(
            "Base64 디코딩은 성공했으나, 유효한 이미지 데이터가 아닐 수 있습니다 (예: PNG 형식이 아님)."
        )
...
if __name__ == "__main__":
    # 1. JSON 파일 경로 설정
    json_file_path = (
        "src/collected_rl_dataset (2).json"  # 실제 JSON 파일 경로로 수정하세요.
    )
        ...
        with open(json_file_path, "r", encoding="utf-8") as f:
            ...
            if data_list and isinstance(data_list, list) and len(data_list) > 0:
                ...
                if isinstance(first_item, dict) and "image_png_base64" in first_item:
                    ...
                    print(
                        f"JSON 파일의 첫 번째 항목에 'image_png_base64' 키가 없거나 형식이 잘못되었습니다."
                    )
            else:
                print(
                    f"JSON 파일 '{json_file_path}'이 비어있거나 예상한 데이터 구조가 아닙니다."
                )
    ...
    if base64_image_data_from_file:
        ...
        decode_base64_to_image_file(
            base64_image_data_from_file, "output_image_from_json.png"
        )
    else:
