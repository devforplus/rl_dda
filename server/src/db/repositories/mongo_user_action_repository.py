...
class MongoUserActionRepository:
    """사용자 액션을 관리하는 리포지토리 클래스입니다."""

    def __init__(self):
        db = get_db()
        self.collection = db[COLLECTION_NAMES["USER_ACTION"]]
    ...
    async def create_user_action(
        ...
        raise Exception("사용자 액션 생성 실패")

    async def get_user_action(self, action_id: str) -> Optional[Dict[str, Any]]:
        """액션 ID로 사용자 액션을 조회합니다."""
    ...
    async def get_action_types_by_session(self, session_id: str) -> List[str]:
        """세션에서 사용된 액션 타입 목록을 조회합니다."""
        pipeline = [
    ...
    async def delete_user_action(self, action_id: str) -> Optional[Dict[str, Any]]:
        """사용자 액션을 삭제합니다."""
        if not ObjectId.is_valid(action_id):
            return None
    ...
    async def delete_session_actions(self, session_id: str) -> int:
        """세션의 모든 액션을 삭제합니다."""
        result = await self.collection.delete_many({"sessionId": session_id})
        return result.deleted_count

    async def count_session_actions(
